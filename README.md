...
- 图片任务开始前确认是否需要口播；需要时先确认风格与时长
- 图片完成并通过检查后，再按实际文件名生成一图一段的口播时间轴
- 保持角色、线条、配色和留白一致
...
如果请求实际生成图片，Skill 会先确认是否需要配套口播。确认需要后，先确定口播风格和总时长；最终口播在图片全部生成、检查和命名完成后创建，并以表格列出时间、图片文件、画面观点、口播内容和屏幕短字。
